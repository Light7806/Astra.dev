...
class DevOpsReward(BaseModel):
    total: float = Field(0.0, gt=-1.0, lt=1.0)
    task_progress: float = Field(0.0)
    efficiency_penalty: float = Field(0.0)
    build_result: float = Field(0.0)
    safety_penalty: float = Field(0.0)
